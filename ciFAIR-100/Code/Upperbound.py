# initial trainloader is random, varied classseq
import os
import torch
...
class cifair100module():
    ...
    def setup(self):
        trainsetlist = [self.trainsetdict[self.class_seq[x]] for x in range(5*self.task)]
        ...
        valsetlist = [self.valsetdict[self.class_seq[x]] for x in range(5*self.task)]
...
def main():
    ...
    wandb.init(project="h3proj", group=args.groupname, config={"seed":str(args.seed), "classseq":"Class Seq "+str(args.classseq)}, name=currentfileseed)
    
    #trainer
    net = Net() 
    dm = cifair100module(classseq=args.classseq)
    wandb.config["fullclassseq"]=str(dm.class_seq)
