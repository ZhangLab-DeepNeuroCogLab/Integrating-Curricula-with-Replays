...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            temptransform = [(transform(x[0]),x[1]) for x in exemplerlis]
            ...
            exemplerlis = [x for _, x in sorted(zip(keys, exemplerlis), key=lambda x:x[0], reverse=False)]
    ...
    os.mkdir(f"../Results/VaryReplayClassSeq/ClusterInstanceez2hard-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryReplayClassSeq/ClusterInstanceez2hard-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryReplayClassSeq/ClusterInstanceez2hard-seed{seed}/Total Accuracy.pkl")
