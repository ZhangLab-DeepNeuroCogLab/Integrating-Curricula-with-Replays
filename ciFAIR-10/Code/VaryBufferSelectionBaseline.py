...
def run(gpu, seed, shuffleseed):
    ...
    os.mkdir(f"../Results/VaryBufferSelection/shuffleseed{shuffleseed}-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryBufferSelection/shuffleseed{shuffleseed}-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryBufferSelection/shuffleseed{shuffleseed}-seed{seed}/Total Accuracy.pkl")
      
import sys
shuffleseed = [1, 10, 100, 1000, 10000, 5, 50, 500, 5000, 50000]
for i in shuffleseed:
    run(gpu=int(sys.argv[1]), seed=int(sys.argv[2]), shuffleseed=i)