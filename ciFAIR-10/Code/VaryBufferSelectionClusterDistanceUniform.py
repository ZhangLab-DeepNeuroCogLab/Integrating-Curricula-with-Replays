...
def run(gpu, seed):
    def selectuniformly(lis, samplelen):
        i = 0
        count = 0
        lislen = len(lis)
        tenmplis = []
        while count < samplelen:
            tenmplis.append(lis[i])
            incre = (lislen-i)//(samplelen-count)
    ...
    pretrainedweights = VGG16_Weights.DEFAULT
    ...
    my_model = FeatureExtractor(vgg16(weights=pretrainedweights))
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            buffersizeperclass = self.buffersize//self.task//2

            for i in orginaldata:
                listoffeaturebyinstance = []
                temptransform = [(transform(x[0]),x[1]) for x in orginaldata[i]]
                tempdataloader = DataLoader(temptransform, batch_size=1, shuffle=False)
                ...
                for k in range(lenoflist):
                    for j in range(lenoflist):
                        matrix[k][j] = np.linalg.norm(listoffeaturebyinstance[k]-listoffeaturebyinstance[j])

                keys = np.sum(matrix, axis=0)
                tobeadded = [x for _, x in sorted(zip(keys, orginaldata[i]), key=lambda x:x[0], reverse=False)]
                self.exemplers[i] = selectuniformly(lis=tobeadded, samplelen=buffersizeperclass)

            for i in self.exemplers:
                self.exemplers[i] = selectuniformly(lis=self.exemplers[i], samplelen=buffersizeperclass)
    ...
    os.mkdir(f"../Results/VaryBufferSelection/clusterdistanceuniform-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryBufferSelection/clusterdistanceuniform-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryBufferSelection/clusterdistanceuniform-seed{seed}/Total Accuracy.pkl")
