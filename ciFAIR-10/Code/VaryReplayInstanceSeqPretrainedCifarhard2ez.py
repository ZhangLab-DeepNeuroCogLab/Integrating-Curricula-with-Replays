...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            tempdataloader = DataLoader(exemplerlis, batch_size=1, shuffle=False)
    ...
    os.mkdir(f"../Results/VaryReplayClassSeq/Instancehard2ez-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryReplayClassSeq/Instancehard2ez-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryReplayClassSeq/Instancehard2ez-seed{seed}/Total Accuracy.pkl")
