...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            toreturn = DataLoader(orginaldatalis, batch_size=32, shuffle=False)
    ...
    os.mkdir(f"../Results/CatastrophicForgetting/seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/CatastrophicForgetting/seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/CatastrophicForgetting/seed{seed}/Total Accuracy.pkl")
      
import sys
shuffleseed = [5, 50, 500, 5000, 50000]
