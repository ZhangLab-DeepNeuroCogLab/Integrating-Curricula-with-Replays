...
def run(gpu, dataseqseed, shuffleseed):
    torch.use_deterministic_algorithms(True)
    torch.backends.cudnn.benchmark = False 
    torch.manual_seed(dataseqseed)
    random.seed(dataseqseed)
    np.random.seed(dataseqseed)
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            listof1and0s = [1]*10000+[0]*1200
            random.seed(shuffleseed)
            random.shuffle(listof1and0s)

            if self.task != 1:
                for i in listof1and0s:
                    if i == 1:
                        outputdata.append(orginaldatalis.pop(0))
                    else:
                        outputdata.append(exemplerlis.pop(0))
            else:
                outputdata = orginaldatalis
    ...
    os.mkdir(f"../Results/VaryInterleave/shuffleseed{shuffleseed}-seed{dataseqseed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryInterleave/shuffleseed{shuffleseed}-seed{dataseqseed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryInterleave/shuffleseed{shuffleseed}-seed{dataseqseed}/Total Accuracy.pkl")
      
import sys
shuffleseed = [1, 10, 100, 1000, 10000, 5, 50, 500, 5000, 50000]
for i in shuffleseed:
    run(gpu=int(sys.argv[1]), dataseqseed=int(sys.argv[2]), shuffleseed=i)
