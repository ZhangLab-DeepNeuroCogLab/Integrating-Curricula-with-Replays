...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            for i in self.exemplers:
                # tempdataloader = DataLoader(self.exemplers[i], batch_size=len(self.exemplers[i]), shuffle=False)
                tempdataloader = DataLoader(self.exemplers[i], batch_size=32, shuffle=False)
                sumprobs = 0
    ...
    os.mkdir(f"../Results/VaryReplayClassSeq/hard2ez-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryReplayClassSeq/hard2ez-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryReplayClassSeq/hard2ez-seed{seed}/Total Accuracy.pkl")
