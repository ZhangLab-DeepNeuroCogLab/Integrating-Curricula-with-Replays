...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            exemplerlis = dict(sorted(self.exemplers.items(), key=lambda x:keys[classes.index(x[0])], reverse=False))
    ...
    os.mkdir(f"../Results/VaryReplayClassSeq/Clusterez2hard-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryReplayClassSeq/Clusterez2hard-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryReplayClassSeq/Clusterez2hard-seed{seed}/Total Accuracy.pkl")
