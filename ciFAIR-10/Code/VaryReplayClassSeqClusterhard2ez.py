...
def run(gpu, seed):
    ...
    class Net(nn.Module):
        ...
        def addandoutputexemplers(self, orginaldata):
            ...
            for i in self.exemplers:
                ...
                tempdataloader = DataLoader(temptransform, batch_size=32, shuffle=False)
        
                sumoffeature = np.zeros(4096)
                ...
                for data in tempdataloader:
                    ...
                    sumoffeature += tempsum
                listoffeaturebyclass.append(sumoffeature/len(self.exemplers[i]))
            ...
            exemplerlis = dict(sorted(self.exemplers.items(), key=lambda x:keys[classes.index(x[0])], reverse=True))
            exemplerlis =  [j for i in exemplerlis.values() for j in i]
            ...
            for i in orginaldata:
                self.exemplers[i] = random.sample(orginaldata[i], self.buffersize//self.task//2)
            for i in self.exemplers:
    ...
    os.mkdir(f"../Results/VaryReplayClassSeq/Clusterhard2ez-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryReplayClassSeq/Clusterhard2ez-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryReplayClassSeq/Clusterhard2ez-seed{seed}/Total Accuracy.pkl")
      
import sys
# l = [1, 10, 100, 1000, 10000]
# l = [1, 10, 100, 1000, 10000, 5, 50, 500, 5000, 50000]
l = [7, 70, 700, 7000, 70000]
