...
def run(gpu, seed, numdiv):
    ...
    class Net(nn.Module):
        def __init__(self, nodes, buffersize, numdiv):
            super().__init__()
            self.exemplers = {}
    ...
    os.mkdir(f"../Results/VaryInterleave/numdiv{numdiv}-seed{seed}/")
    ...
    dfaccuracybybatch.to_pickle(f"../Results/VaryInterleave/numdiv{numdiv}-seed{seed}/Accuracy by class.pkl")
      
    x1=range(len(totalaccuracy))
    y1=totalaccuracy
    dftotalaccuracy = pd.DataFrame(data={"Accuracy":y1}, index=x1) 
    dftotalaccuracy.to_pickle(f"../Results/VaryInterleave/numdiv{numdiv}-seed{seed}/Total Accuracy.pkl")
      
import sys
l = [1, 4, 8, 16, 25, 50, 100, 200, 400]
for i in l:
    run(gpu=int(sys.argv[1]), seed=int(sys.argv[2]), numdiv=i)
